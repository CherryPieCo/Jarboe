...
<fieldset style="padding: 14px 14px 5px;">
    ...
    @foreach ($info['rights'] as $id => $caption)
        ...
        <div class="col col-3">
            ...
            <label class="radio">
                <input type="radio" name="pattern[group_permissions][{{$ident}}][{{$id}}]" value="allow" @if($perm === true) checked="checked" @endif>
                <i></i>Разрешить</label>
            <label class="radio">
                <input type="radio" name="pattern[group_permissions][{{$ident}}][{{$id}}]" value="deny" @if($perm === false) checked="checked" @endif>
                <i></i>Запретить</label>
        </div>
    @endforeach
        </div>
